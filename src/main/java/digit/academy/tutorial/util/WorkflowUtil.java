...
public class WorkflowUtil {
    ...
    */
    private ProcessInstance getProcessInstanceForWorkflow(RequestInfo requestInfo, String tenantId,
        ...
        processInstance.setDocuments(toWorkflowDocuments(workflow.getDocuments()));
    ...
    */
    public Map<String, Workflow> getWorkflow(List<ProcessInstance> processInstances) {
        ...
        processInstances.forEach(processInstance -> {
            ...
            Workflow workflow = Workflow.builder()
                ...
                .documents(toApiDocuments(processInstance.getDocuments()))
                .build();
    ...
    private List<digit.models.coremodels.Document> toWorkflowDocuments(
        List<digit.academy.tutorial.web.models.Document> documents) {

        if (CollectionUtils.isEmpty(documents))
            return Collections.emptyList();

        return documents.stream()
            .map(document -> mapper.convertValue(document, digit.models.coremodels.Document.class))
            .collect(Collectors.toList());
    }

    private List<digit.academy.tutorial.web.models.Document> toApiDocuments(
        List<digit.models.coremodels.Document> documents) {

        if (CollectionUtils.isEmpty(documents))
            return Collections.emptyList();

        return documents.stream()
            .map(document -> mapper.convertValue(document, digit.academy.tutorial.web.models.Document.class))
            .collect(Collectors.toList());
    }
